...
// TODO: deduplicate between apis

/// Read only access to the VoxelWorld
#[derive(SystemParam)]
pub struct VoxelWorldRead<'w, C: VoxelWorldConfig> {
    chunk_map: Res<'w, ChunkMap<C, <C as VoxelWorldConfig>::MaterialIndex>>,
    modified_voxels: Res<'w, ModifiedVoxels<C, <C as VoxelWorldConfig>::MaterialIndex>>,
    #[allow(unused)]
    configuration: Res<'w, C>,
...
impl<C: VoxelWorldConfig> VoxelWorldRead<'_, C> {
    /// Get the voxel at the given position. The voxel will be WorldVoxel::Unset if there is no voxel at that position
    pub fn get_voxel(&self, position: IVec3) -> WorldVoxel<C::MaterialIndex> {
        self.get_voxel_fn()(position)
    }
    ...
    /// Convert the read only VoxelWorld into a VoxelWorld you "own"
    /// The internals are still shared.
    pub fn to_owned(&self) -> VoxelWorldReadOwned<C> {
        VoxelWorldReadOwned {
...
/// Read only access to the VoxelWorld
#[derive(Clone)]
pub struct VoxelWorldReadOwned<C: VoxelWorldConfig> {
    chunk_map: ChunkMap<C, <C as VoxelWorldConfig>::MaterialIndex>,
    modified_voxels: ModifiedVoxels<C, <C as VoxelWorldConfig>::MaterialIndex>,
    _marker: PhantomData<C>,
}

impl<C: VoxelWorldConfig> VoxelWorldReadOwned<C> {
    /// Create an owned reader directly from a World.
    ///
    /// Useful for accessing voxel data outside of Bevy's SystemParam context,
    /// such as in HTN planning systems that work with raw World access.
    pub fn from_world(world: &World) -> Self {
        let chunk_map = world.resource::<ChunkMap<C, C::MaterialIndex>>();
        let modified_voxels = world.resource::<ModifiedVoxels<C, C::MaterialIndex>>();
        Self {
